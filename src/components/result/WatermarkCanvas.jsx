 ...
 * Uses fetch-as-blob to bypass CORS restrictions on canvas.
 */
export default function WatermarkCanvas({ imageUrl, onReady }) {
  ...
  useEffect(() => {
    ...
    // Fetch as blob first to bypass CORS restrictions on canvas taint
    fetch(imageUrl)
      .then(r => r.blob())
      .then(blob => {
        const blobUrl = URL.createObjectURL(blob);
        applyWatermark(blobUrl);
      })
      .catch(() => onReady(imageUrl));
  }, [imageUrl]);
