...
const featuresData = {
  ...
  en: [
    { icon: Sparkles, title: 'AI Sketch Generation', desc: 'Transform any product photo into a professional industrial design sketch in seconds.' },
    { icon: Palette, title: 'Pantone Colors', desc: 'Specify your Pantone palette and the AI will faithfully respect your brand colors.' },
    { icon: Layers, title: 'Multiple Styles', desc: 'Marker render, line drawing, pencil sketch, technical drawing, watercolor and much more.' },
    { icon: Download, title: 'Portfolio-ready', desc: 'Download in high quality and use directly in presentations, pitches and design portfolios.' },
  ],
};
    ...
    <div className="min-h-screen bg-background flex flex-col">
      ...
      <header className="sticky top-0 z-10 bg-background/80 backdrop-blur border-b border-border px-6 py-4 flex items-center justify-between" style={{ paddingTop: 'calc(1rem + env(safe-area-inset-top))' }}>
        ...
        <Button onClick={handleLogin} size="sm" className="gap-2">
          {lang === 'it' ? 'Accedi' : 'Sign in'} <ArrowRight className="w-3.5 h-3.5" />
        </Button>
      </header>
        ...
        <section className="max-w-3xl mx-auto px-6 py-20 text-center space-y-6">
          <motion.div initial={{ opacity: 0, y: 20 }} animate={{ opacity: 1, y: 0 }} transition={{ duration: 0.6 }}>
            <h1 className="text-4xl sm:text-5xl font-bold tracking-tight leading-tight">
              {lang === 'it' ? <>Sketch di industrial design<br /><span className="text-accent">generati dall'AI</span></> : <>Industrial design sketches<br /><span className="text-accent">generated by AI</span></>}
            </h1>
            <p className="mt-5 text-muted-foreground text-base sm:text-lg max-w-xl mx-auto">
              {lang === 'it'
                ...
                : 'Upload a product photo and get a professional sketch in seconds — with Pantone colors, multiple perspectives and graphic styles. Perfect for designers, agencies and students.'}
            </p>
          </motion.div>
          ...
          <motion.div initial={{ opacity: 0, y: 10 }} animate={{ opacity: 1, y: 0 }} transition={{ delay: 0.2, duration: 0.5 }}>
            <Button onClick={handleLogin} size="lg" className="gap-2 bg-accent hover:bg-accent/90 text-accent-foreground font-semibold px-8 rounded-xl">
              ...
              {lang === 'it' ? 'Inizia subito — è gratis' : 'Get started — it\'s free'}
            </Button>
          </motion.div>
      ...
      <footer className="border-t border-border px-6 py-5 flex flex-col sm:flex-row items-center justify-between gap-3 text-xs text-muted-foreground">
        ...
        <div className="flex gap-4">
          ...
          <a href="/terms" className="hover:text-foreground underline underline-offset-2 transition-colors">{lang === 'it' ? 'Termini e Condizioni' : 'Terms of Service'}</a>
          <a href="/support" className="hover:text-foreground underline underline-offset-2 transition-colors">{lang === 'it' ? 'Supporto' : 'Support'}</a>
          <a href="/marketing" className="hover:text-foreground underline underline-offset-2 transition-colors">Media Kit</a>
          <a href="/age-rating" className="hover:text-foreground underline underline-offset-2 transition-colors">{lang === 'it' ? 'Adeguatezza dell\'età' : 'Age Rating'}</a>
        </div>
      </footer>
    </div>);
